...
public class ChoisirMedecinActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_choisir_medecin); // Assure-toi que ce layout existe

        recyclerView = findViewById(R.id.recyclerMedecins);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        // Données fictives avec plus d'informations
        ...
        adapter = new MedecinAdapter(listeMedecins, medecin -> {
            // Clic sur un médecin → ouvrir ProfilMedecinActivity avec les détails
            Intent intent = new Intent(this, ProfilMedecinActivity.class);
            intent.putExtra("nom", medecin.getNom());
            intent.putExtra("specialite", medecin.getSpecialite());
            intent.putExtra("image", medecin.getImageResId());
            intent.putExtra("rating", medecin.getRating());
            intent.putExtra("lieuTravail", medecin.getLieuTravail());
            intent.putExtra("experience", medecin.getExperienceAnnees());
